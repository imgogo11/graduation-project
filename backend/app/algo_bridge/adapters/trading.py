...
class RangeMaxModuleResult:
    max_value_scaled: int
    matched_indices: list[int]


@dataclass(frozen=True, slots=True)
class RangeKthModuleResult:
...
@dataclass(frozen=True, slots=True)
class HistoricalDominanceModuleResult:
    dominated_counts: list[int]


def _module_directory() -> Path:
    return get_settings().project_root / "algo-module" / "build" / "python"
...
def load_algo_module():
    ...
    try:
        ...
        try:
            ...
            raise RuntimeError(
                f"{module_name} is not available. Build the algo-module module so it exists under {module_dir}."
            ) from exc


def query_range_max(values_scaled: Sequence[int], left: int, right: int) -> RangeMaxModuleResult:
    module = load_algo_module()
    tree = module.RangeMaxSegmentTree(list(values_scaled))
    result = tree.query_inclusive(left, right)
    return RangeMaxModuleResult(
        max_value_scaled=int(result.max_value_scaled),
        matched_indices=[int(index) for index in result.matched_indices],
    )


def query_range_kth(values_scaled: Sequence[int], left: int, right: int, k: int) -> RangeKthModuleResult:
    module = load_algo_module()
    tree = module.RangeKthPersistentSegmentTree(list(values_scaled))
    result = tree.query_inclusive(left, right, k)
    return RangeKthModuleResult(
...
def query_range_kth_tdigest(values_scaled: Sequence[int], left: int, right: int, k: int) -> RangeKthModuleResult:
